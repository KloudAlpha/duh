package com.loopj.android.http;

import cz.msebera.android.httpclient.Header;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;
import org.json.JSONTokener;
import p001a.C0048o;
/* loaded from: classes.dex */
public class JsonHttpResponseHandler extends TextHttpResponseHandler {
    private static final String LOG_TAG = "JsonHttpRH";
    private boolean useRFC5179CompatibilityMode;

    public JsonHttpResponseHandler() {
        super("UTF-8");
        this.useRFC5179CompatibilityMode = true;
    }

    public boolean isUseRFC5179CompatibilityMode() {
        return this.useRFC5179CompatibilityMode;
    }

    public void onFailure(int i, Header[] headerArr, Throwable th2, JSONObject jSONObject) {
        AsyncHttpClient.log.mo11833w(LOG_TAG, "onFailure(int, Header[], Throwable, JSONObject) was not overriden, but callback was received", th2);
    }

    public void onSuccess(int i, Header[] headerArr, JSONObject jSONObject) {
        AsyncHttpClient.log.mo11834w(LOG_TAG, "onSuccess(int, Header[], JSONObject) was not overriden, but callback was received");
    }

    public Object parseResponse(byte[] bArr) throws JSONException {
        Object obj = null;
        if (bArr == null) {
            return null;
        }
        String responseString = TextHttpResponseHandler.getResponseString(bArr, getCharset());
        if (responseString != null) {
            responseString = responseString.trim();
            if (this.useRFC5179CompatibilityMode) {
                if (responseString.startsWith("{") || responseString.startsWith("[")) {
                    obj = new JSONTokener(responseString).nextValue();
                }
            } else if ((responseString.startsWith("{") && responseString.endsWith("}")) || (responseString.startsWith("[") && responseString.endsWith("]"))) {
                obj = new JSONTokener(responseString).nextValue();
            } else if (responseString.startsWith("\"") && responseString.endsWith("\"")) {
                obj = responseString.substring(1, responseString.length() - 1);
            }
        }
        if (obj != null) {
            return obj;
        }
        return responseString;
    }

    public void setUseRFC5179CompatibilityMode(boolean z) {
        this.useRFC5179CompatibilityMode = z;
    }

    public void onFailure(int i, Header[] headerArr, Throwable th2, JSONArray jSONArray) {
        AsyncHttpClient.log.mo11833w(LOG_TAG, "onFailure(int, Header[], Throwable, JSONArray) was not overriden, but callback was received", th2);
    }

    public void onSuccess(int i, Header[] headerArr, JSONArray jSONArray) {
        AsyncHttpClient.log.mo11834w(LOG_TAG, "onSuccess(int, Header[], JSONArray) was not overriden, but callback was received");
    }

    public JsonHttpResponseHandler(String str) {
        super(str);
        this.useRFC5179CompatibilityMode = true;
    }

    @Override // com.loopj.android.http.TextHttpResponseHandler
    public void onFailure(int i, Header[] headerArr, String str, Throwable th2) {
        AsyncHttpClient.log.mo11833w(LOG_TAG, "onFailure(int, Header[], String, Throwable) was not overriden, but callback was received", th2);
    }

    @Override // com.loopj.android.http.TextHttpResponseHandler
    public void onSuccess(int i, Header[] headerArr, String str) {
        AsyncHttpClient.log.mo11834w(LOG_TAG, "onSuccess(int, Header[], String) was not overriden, but callback was received");
    }

    @Override // com.loopj.android.http.TextHttpResponseHandler, com.loopj.android.http.AsyncHttpResponseHandler
    public final void onFailure(final int i, final Header[] headerArr, final byte[] bArr, final Throwable th2) {
        if (bArr != null) {
            Runnable runnable = new Runnable() { // from class: com.loopj.android.http.JsonHttpResponseHandler.2
                @Override // java.lang.Runnable
                public void run() {
                    try {
                        final Object parseResponse = JsonHttpResponseHandler.this.parseResponse(bArr);
                        JsonHttpResponseHandler.this.postRunnable(new Runnable() { // from class: com.loopj.android.http.JsonHttpResponseHandler.2.1
                            @Override // java.lang.Runnable
                            public void run() {
                                if (!JsonHttpResponseHandler.this.useRFC5179CompatibilityMode && parseResponse == null) {
                                    RunnableC21962 runnableC21962 = RunnableC21962.this;
                                    JsonHttpResponseHandler.this.onFailure(i, headerArr, (String) null, th2);
                                    return;
                                }
                                Object obj = parseResponse;
                                if (obj instanceof JSONObject) {
                                    RunnableC21962 runnableC219622 = RunnableC21962.this;
                                    JsonHttpResponseHandler.this.onFailure(i, headerArr, th2, (JSONObject) obj);
                                } else if (obj instanceof JSONArray) {
                                    RunnableC21962 runnableC219623 = RunnableC21962.this;
                                    JsonHttpResponseHandler.this.onFailure(i, headerArr, th2, (JSONArray) obj);
                                } else if (obj instanceof String) {
                                    RunnableC21962 runnableC219624 = RunnableC21962.this;
                                    JsonHttpResponseHandler.this.onFailure(i, headerArr, (String) obj, th2);
                                } else {
                                    RunnableC21962 runnableC219625 = RunnableC21962.this;
                                    JsonHttpResponseHandler jsonHttpResponseHandler = JsonHttpResponseHandler.this;
                                    int i2 = i;
                                    Header[] headerArr2 = headerArr;
                                    StringBuilder m14987g = C0048o.m14987g("Unexpected response type ");
                                    m14987g.append(parseResponse.getClass().getName());
                                    jsonHttpResponseHandler.onFailure(i2, headerArr2, new JSONException(m14987g.toString()), (JSONObject) null);
                                }
                            }
                        });
                    } catch (JSONException e) {
                        JsonHttpResponseHandler.this.postRunnable(new Runnable() { // from class: com.loopj.android.http.JsonHttpResponseHandler.2.2
                            @Override // java.lang.Runnable
                            public void run() {
                                RunnableC21962 runnableC21962 = RunnableC21962.this;
                                JsonHttpResponseHandler.this.onFailure(i, headerArr, e, (JSONObject) null);
                            }
                        });
                    }
                }
            };
            if (!getUseSynchronousMode() && !getUsePoolThread()) {
                new Thread(runnable).start();
                return;
            } else {
                runnable.run();
                return;
            }
        }
        AsyncHttpClient.log.mo11836v(LOG_TAG, "response body is null, calling onFailure(Throwable, JSONObject)");
        onFailure(i, headerArr, th2, (JSONObject) null);
    }

    @Override // com.loopj.android.http.TextHttpResponseHandler, com.loopj.android.http.AsyncHttpResponseHandler
    public final void onSuccess(final int i, final Header[] headerArr, final byte[] bArr) {
        if (i != 204) {
            Runnable runnable = new Runnable() { // from class: com.loopj.android.http.JsonHttpResponseHandler.1
                @Override // java.lang.Runnable
                public void run() {
                    try {
                        final Object parseResponse = JsonHttpResponseHandler.this.parseResponse(bArr);
                        JsonHttpResponseHandler.this.postRunnable(new Runnable() { // from class: com.loopj.android.http.JsonHttpResponseHandler.1.1
                            @Override // java.lang.Runnable
                            public void run() {
                                if (!JsonHttpResponseHandler.this.useRFC5179CompatibilityMode && parseResponse == null) {
                                    RunnableC21931 runnableC21931 = RunnableC21931.this;
                                    JsonHttpResponseHandler.this.onSuccess(i, headerArr, (String) null);
                                    return;
                                }
                                Object obj = parseResponse;
                                if (obj instanceof JSONObject) {
                                    RunnableC21931 runnableC219312 = RunnableC21931.this;
                                    JsonHttpResponseHandler.this.onSuccess(i, headerArr, (JSONObject) obj);
                                } else if (obj instanceof JSONArray) {
                                    RunnableC21931 runnableC219313 = RunnableC21931.this;
                                    JsonHttpResponseHandler.this.onSuccess(i, headerArr, (JSONArray) obj);
                                } else if (obj instanceof String) {
                                    if (JsonHttpResponseHandler.this.useRFC5179CompatibilityMode) {
                                        RunnableC21931 runnableC219314 = RunnableC21931.this;
                                        JsonHttpResponseHandler.this.onFailure(i, headerArr, (String) parseResponse, new JSONException("Response cannot be parsed as JSON data"));
                                        return;
                                    }
                                    RunnableC21931 runnableC219315 = RunnableC21931.this;
                                    JsonHttpResponseHandler.this.onSuccess(i, headerArr, (String) parseResponse);
                                } else {
                                    RunnableC21931 runnableC219316 = RunnableC21931.this;
                                    JsonHttpResponseHandler jsonHttpResponseHandler = JsonHttpResponseHandler.this;
                                    int i2 = i;
                                    Header[] headerArr2 = headerArr;
                                    StringBuilder m14987g = C0048o.m14987g("Unexpected response type ");
                                    m14987g.append(parseResponse.getClass().getName());
                                    jsonHttpResponseHandler.onFailure(i2, headerArr2, new JSONException(m14987g.toString()), (JSONObject) null);
                                }
                            }
                        });
                    } catch (JSONException e) {
                        JsonHttpResponseHandler.this.postRunnable(new Runnable() { // from class: com.loopj.android.http.JsonHttpResponseHandler.1.2
                            @Override // java.lang.Runnable
                            public void run() {
                                RunnableC21931 runnableC21931 = RunnableC21931.this;
                                JsonHttpResponseHandler.this.onFailure(i, headerArr, e, (JSONObject) null);
                            }
                        });
                    }
                }
            };
            if (!getUseSynchronousMode() && !getUsePoolThread()) {
                new Thread(runnable).start();
                return;
            } else {
                runnable.run();
                return;
            }
        }
        onSuccess(i, headerArr, new JSONObject());
    }

    public JsonHttpResponseHandler(boolean z) {
        super("UTF-8");
        this.useRFC5179CompatibilityMode = z;
    }

    public JsonHttpResponseHandler(String str, boolean z) {
        super(str);
        this.useRFC5179CompatibilityMode = z;
    }
}
